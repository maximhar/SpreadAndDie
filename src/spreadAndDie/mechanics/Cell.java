...
public class Cell {
	...
	public boolean isNearDisease() {
		return this.left.isDiseased() || this.right.isDiseased()
				|| this.top.isDiseased() || this.bottom.isDiseased();
	}
	...
	public void endTick() {
		if (this.beingInfected)
			this.makeInfected();
		if (this.beingDiseased)
			this.makeDiseased();
		this.beingDiseased = this.beingInfected = false;
	}

	public static Cell createWithNeighbours(Cell topCell, Cell leftCell) {
		Cell c = new Cell().setTop(topCell).setLeft(leftCell)
				.setBottom(Cell.border).setRight(Cell.border);
